    ...
    <div class="container">
        ...
        <div style="display:flex;flex-direction:column;gap:2rem">
            ...
            <div class="card" style="display:flex;flex-direction:row;overflow:hidden;min-height:280px">
                <!-- Image -->
                <div style="width:340px;flex-shrink:0;position:relative;background:linear-gradient(135deg,var(--jungle-dark),var(--jungle-mid));display:flex;align-items:center;justify-content:center">
                    <?php if ($room['primary_image']): ?>
                        <img src="<?= SITE_URL ?>/assets/images/<?= htmlspecialchars($room['primary_image']) ?>" alt="" style="width:100%;height:100%;object-fit:cover">
                    <?php else: ?>
                        <i class="fas fa-<?= $room['category_id']==1?'tree':($room['category_id']==2?'water':($room['category_id']==3?'mountain':'compass')) ?>" style="font-size:5rem;color:rgba(255,255,255,.15)"></i>
                    <?php endif; ?>
                    <?php if ($room['featured']): ?>
                    <div style="position:absolute;top:1rem;left:1rem;background:var(--ochre);color:white;padding:.25rem .7rem;border-radius:20px;font-size:.72rem;font-weight:700;font-family:var(--font-head);letter-spacing:.08em;text-transform:uppercase">
                ...
                <div style="flex:1;padding:2rem;display:flex;flex-direction:column;justify-content:space-between">
                    ...
                    <div style="display:flex;align-items:center;justify-content:space-between;margin-top:1.5rem;padding-top:1.25rem;border-top:1px solid var(--stone);flex-wrap:wrap;gap:.75rem">
                        ...
                        <div style="display:flex;gap:.75rem">
                            ...
                            <a href="<?= SITE_URL ?>/guest/book.php?room_id=<?=$room['id']?>&check_in=<?=$check_in?>&check_out=<?=$check_out?>&guests=<?=$guests?>" class="btn btn-primary btn-sm">
